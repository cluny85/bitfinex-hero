...
module.exports = () => {
    ...
    if (Array.isArray(msg)) {
      if (typeof msg === 'object' && msg.length > 1) {
        if (msg[0] > 0 && typeof msg[1] !== 'string') {
          eventListener.handleEvents(msg);
        } else {
          // TODO: hacer algo con los tipos de eventos que traen en la segunda posicion un string tipo: ow
          const type = msg[1];
          try {
            eventActions[type]('Cazado tipo: ' + type, msg);
          } catch (err) {
            error(`Evento ${type} no contemplado aun. `);
          }
        }
      }
      // ws: 'WALLET/SNAPSHOT',
  ...
  function handleEventMessage(message) {
    try {
      // TODO: continuar con la gestión de los tickers subscritos para gestionar
      // el chanId: y asociarlo listener que disparará mis eventos customizados al
      // exterior
      eventHandler[message.event]('::: handeled: '+message.event); // message);
    } catch (err) {
      error(`${message.event} event type not listed jet.`);
    }
  }
